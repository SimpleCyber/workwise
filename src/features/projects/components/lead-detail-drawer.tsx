...
export function LeadDetailDrawer({ leadId, onClose }: LeadDetailDrawerProps) {
  ...
  const callLogs = useQuery(api.sales.getCallLogs, leadId ? { leadId } : "skip");
  ...
  const handleLogCall = async (disposition: string) => {
    ...
    if ((disposition === "retry_scheduled" || disposition === "meeting_scheduled") && !selectedDisposition) {
    ...
    try {
      ...
      await logCall({
        ...
        nextActionAt: nextActionDate ? new Date(nextActionDate).getTime() : undefined,
      });
      ...
      <SheetContent className="w-[450px] sm:w-[540px] sm:max-w-[100vw] flex flex-col p-6 bg-background">
        ...
        <SheetHeader className="mb-6 flex flex-row items-start justify-between border-b pb-4 space-y-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <SheetTitle className="text-lg leading-tight">{lead?.name || "Unknown Lead"}</SheetTitle>
        ...
        {/* Log this call Box */}
        <div className="mb-8">
          <h3 className="text-sm font-medium text-muted-foreground mb-3">Log this call</h3>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
             ...
             <div className="bg-muted/30 p-3 rounded-lg border border-border animate-in fade-in zoom-in-95">
                <p className="text-xs font-semibold mb-2">
                  Select {selectedDisposition === "meeting_scheduled" ? "meeting" : "retry"} date & time:
                </p>
                <Input 
                   type="datetime-local" 
                ...
                <div className="flex gap-2 justify-end">
                   <Button size="sm" variant="ghost" onClick={cancelSelection} className="h-8 text-xs">Cancel</Button>
                   <Button size="sm" onClick={() => handleLogCall(selectedDisposition)} disabled={!nextActionDate || isLoading} className="h-8 text-xs">
                     Save Log
                   </Button>
                </div>
             </div>
        ...
        <div className="flex-1 overflow-y-auto pr-2">
          <h3 className="text-sm font-medium text-muted-foreground mb-4">Call history</h3>
          
          <div className="relative border-l border-border ml-1.5 space-y-6 pb-4">
            {callLogs?.length === 0 && (
              <div className="pl-6 text-sm text-muted-foreground py-2">No history yet.</div>
            )}
            ...
            {callLogs?.map((log: any) => {
              ...
              return (
                <div key={log._id} className="relative pl-6">
                  <div className={`absolute -left-1.5 top-1.5 h-3 w-3 rounded-full border-2 border-background ${dotColor}`} />
                  <div>
                    <div className="flex items-center gap-1.5">
                      <span className="text-sm font-semibold">{title}</span>
                      <span className="text-sm text-muted-foreground">· {log.agentName}</span>
                    </div>
                    <div className="text-xs text-muted-foreground mt-0.5">
                      {format(new Date(log.timestamp), "MMM d, h:mm a")}
                      {log.nextActionAt && (
                        <span> — Next action: {format(new Date(log.nextActionAt), "MMM d, h:mm a")}</span>
                      )}
                    </div>
                  </div>
                </div>
