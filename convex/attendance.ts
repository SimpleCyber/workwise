...
export const markAbsentUsers = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const attendedMemberIds = new Set(
      existingAttendance.map((record) => record.memberId),
    );
...
export const getAttendanceByDate = query({
  args: {
    ...
    date: v.number(),
    filter: v.optional(
      v.union(v.literal("present"), v.literal("absent"), v.literal("all")),
    ),
  },
  handler: async (ctx, args) => {
    const userId = await getAuthUserId(ctx);
    ...
    if (args.filter === "present") {
      filteredRecords = allAttendanceRecords.filter(
...
export const updateAttendanceStatus = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const currentMember = await ctx.db
      .query("members")
      .withIndex("by_workspace_id_user_id", (q) =>
        q.eq("workspaceId", attendance.workspaceId).eq("userId", userId),
      )
      .unique();
