...
export const getUserNotifications = query({
  args: {
    ...
    category: v.optional(
      v.union(v.literal("all"), v.literal("attendance"), v.literal("projects"), v.literal("dataroom")),
    ),
  },
  handler: async (ctx, args) => {
    ...
    const notificationsWithData = await Promise.all(
      notifications.map(async (notification) => {
        ...
        const actionUser = notification.actionBy ? await ctx.db.get(notification.actionBy) : null

        // Check if there are any approval/rejection notifications for this attendance
        let hasBeenProcessed = false
        if (notification.type === "attendance_submitted" && notification.relatedId) {
          const processedNotifications = await ctx.db
            .query("notifications")
            .filter((q) =>
              q.and(
                q.eq(q.field("relatedId"), notification.relatedId),
                q.or(q.eq(q.field("type"), "attendance_approved"), q.eq(q.field("type"), "attendance_rejected")),
              ),
            )
            .first()
...
export const getAllNotifications = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Apply filters
    if (args.workspaceId) {
      notifications = notifications.filter((n) => n.workspaceId === args.workspaceId)
    }
    ...
    if (args.emailSent !== undefined) {
      notifications = notifications.filter((n) => n.sendedmail === args.emailSent)
    }
...
export const getNotificationStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    const stats = {
      ...
      recentActivity: notifications.filter((n) => Date.now() - n.createdAt < 24 * 60 * 60 * 1000).length, // Last 24 hours
    }
    ...
    const workspaceIds = Array.from(new Set(notifications.map((n) => n.workspaceId)))
...
export const markAllAsRead = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await Promise.all(unreadNotifications.map((notification) => ctx.db.patch(notification._id, { isRead: true })))
